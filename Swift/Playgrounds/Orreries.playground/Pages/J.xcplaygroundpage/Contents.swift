...
let sunVenusRadius = SCNNode()
sunVenusRadius.position = SCNVector3(x: 4, y: 0, z: 0)
sunVenusRadius.name = "venus-radius"

let venusMaterial = sunMaterial.copy() as! SCNMaterial
venusMaterial.diffuse.contents = "venus.jpg"

let venus = SCNSphere(radius: 0.4)
venus.firstMaterial = venusMaterial

let venusAxis = SCNNode()
let venusNode = SCNNode(geometry: venus)

venusAxis.addChildNode(venusNode)
sunVenusRadius.addChildNode(venusAxis)
sunVenusCollar.addChildNode(sunVenusRadius)

sunVenusCollar.addAnimation(generateAnimation(duration: 20), forKey: "venus orbit")

venusNode.addAnimation(generateAnimation(duration: 1), forKey: "venus spin")
...
sunEarthRadius.position = SCNVector3(x: 6, y: 0, z: 0)
...
let earth = SCNSphere(radius: 0.4)
...
earthMoonRadius.position = SCNVector3(x: 0.7, y: 0, z: 0)
