...
class Utilities {
    ...
    static func createOrbitalPath(forDistance orbit: CGFloat, withObjectRadius radius: CGFloat) -> SCNNode {
        ...
        path.rotation = SCNVector4(x: 1, y: 0, z:0, w: -CGFloat.pi/2.0)
        return path
    }
...
public class Ring {
    ...
    public init(withInnerRadiusAt innerRadius: CGFloat, andOuterRadiusAt outerRadius: CGFloat, usingImge image: String?) {
        ...
        topMaterial.diffuse.contents = image ?? NSColor.white
        topMaterial.diffuse.mipFilter = .linear
        topMaterial.diffuse.contentsTransform = SCNMatrix4MakeRotation(CGFloat.pi / 4.0, 0, 0, 1)
        bottomMaterial = SCNMaterial()
        bottomMaterial.diffuse.contents = image ?? NSColor.white
        innerMaterial = SCNMaterial()
        innerMaterial.diffuse.contents = NSColor.white
        outerMaterial = SCNMaterial()
        outerMaterial.diffuse.contents = NSColor.white
